...
__all__ = ['linspace', 'Block', 'bias_model', 'BiasBlock', 'LinearD', 'GenericBlock', 'seasonality_model',
           'SeasonalityBlock', 'trend_model', 'TrendBlock', 'select_block', 'NBeatsNet', 'first_item', 'NBeatsTrainer']
...
class NBeatsTrainer(Callback):
    ...
    def __init__(self, store_b = True, store_f=False,):
        store_attr(self,'store_b,store_f')
    ...
    def after_pred(self):
        if self.store_b:
            total_b = self.pred[1]
            self.pred[2]['total_b'] = first_item(self.learn.dls).after_batch.decode((total_b,))[0]

        if self.store_f:
            total_f = first_item(self.learn.dls).after_batch.decode((self.pred[0],))[0]
            self.pred[2]['total_f'] = total_f

        self.pred[2]['total_b_loss'] = self.learn.loss_func(total_b.float(), *self.learn.xb, reduction='none').mean(-1)
