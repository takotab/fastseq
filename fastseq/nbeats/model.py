...
__all__ = ['linspace', 'Block', 'seasonality_model', 'SeasonalityBlock', 'trend_model', 'TrendBlock', 'select_block',
           'default_thetas', 'NBeatsNet', 'bias_model', 'BiasBlock', 'LinearD', 'GenericBlock']
...
class Block(Module):
    def __init__(self, fnc_f, fnc_b=None):
        ...
        if self.att:
            self.att = LinBnDrop(sizes[-1], self.thetas_dim)
...
class BiasBlock(Block):
    def __init__(
        self, layers:L, device, thetas_dim=2, lookback=10, horizon=5, use_bn=True, bn_final=False, ps:L=None, att=True
    ):
        share_thetas=True
        assert thetas_dim <= 2, f"thetas_dim for BaisBlock must be < than 2, is now {thetas_dim}"
        store_attr(self,"device,layers,thetas_dim,use_bn,ps,lookback,horizon,bn_final,share_thetas,att" )
