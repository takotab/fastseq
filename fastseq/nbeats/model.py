...
__all__ = ['linspace', 'make_base', 'Block', 'seasonality_model', 'SeasonalityBlock', 'trend_model', 'TrendBlock',
...
# Cell
class Block(Module):
    def __init__(self, fnc_f, fnc_b=None, base=None):
...
def seasonality_model(thetas, t):
    p = thetas.size()[-1]
    assert p < 12, "thetas_dim is too big."
    p1, p2 = (p // 2, p // 2) if p % 2 == 0 else (p // 2, p // 2 + 1)
    period = ((t.max()-t.min()))*2
#     print(period)
...
class SeasonalityBlock(Block):
    def __init__(
        ...
        bn_final=False, ps:L=None, share_thetas=True, y_range=[-1,1], att=True, scale_exp = 2, stand_alone=False, base = None,
    ):
...
class TrendBlock(Block):
    def __init__(
        ...
        bn_final=False, ps:L=None, share_thetas=True, y_range=[-.1, .1], att = True, scale_exp = 10,stand_alone=False,base=None,
    ):
        store_attr(self,"y_range,device,layers,thetas_dim,use_bn,ps,lookback,horizon,bn_final,share_thetas,att,stand_alone,base" )
...
class NBeatsNet(Module):
    def __init__(
        ...
        store_attr(self,'device,horizon,lookback,layers,nb_blocks_per_stack,share_weights_in_layers,stack_types,thetas_dim,device')
        self.stacks = []
        self._str = "| N-Beats\n"
