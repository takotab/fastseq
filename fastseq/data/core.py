...
__all__ = ['NormalizeSeq', 'NormalizeSeqMulti', 'make_test', 'split_file', 'multithread_f', 'TSSplitter',
           'get_train_valid_ts', 'MTSDataLoaders']
...
# Cell
def _exe_splitter(path, horizon, lookback, valid_pct, splitter=None, num_workers = None):
    files = get_files(path, extensions='.json', folders = False)
    splitter = ifnone(splitter, TSSplitter(folder = 'valid', horizon= horizon + int(valid_pct*horizon),lookback=lookback,
                                keep_lookback = True))
...
def get_train_valid_ts(path, **kwargs):
    if (path / 'valid').exists():
        ...
        train, valid = _exe_splitter(path, **kwargs)
    return train, valid
