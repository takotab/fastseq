# AUTOGENERATED! DO NOT EDIT! File to edit: nbs/05_models.nbeats.ipynb (unless otherwise specified).
        ...
        for k,v in learn.n_beats_trainer.out.items():
            if isinstance(k,str):
                if 'seasonality' in k or 'trend' in k:
                    dct[k]={'mean':np.round(v['attention'].mean().cpu().numpy(),2),
                            'std':np.round(v['attention'].std().cpu().numpy(),2)}
        if df:
